...
from viemonet.models.submodels.meta_classifier_without_rules import MetaClassifier_Without_Rules
from viemonet.config import device, config


class Viemonet_Polarity_Comp(nn.Module):
    def __init__(
        self,
        class_weights,
        label_smoothing=0.1,
    ):
        super(Viemonet_Polarity_Comp, self).__init__()
        self.comment_classifier = CommentClassifier('phobert', 'cnn')
        self.emotion_classifier = EmotionClassifier()
        self.meta_classifier = MetaClassifier_Without_Rules()
