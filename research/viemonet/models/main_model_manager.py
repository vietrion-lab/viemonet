...
from viemonet.models.main_models.viemonet_no_metacls import ViemonetModel_No_MetaCLS
...
from viemonet.models.main_models.viemonet_polarity_comp import Viemonet_Polarity_Comp
...
class MainModelManager:
    ...
    def get_model(self, model_name: str, class_weights=None):
        ...
        if model_name == MAIN_MODEL_LIST[0]:
            ...
            return ViemonetModel_No_MetaCLS(class_weights=class_weights, label_smoothing=self.label_smoothing)
        ...
        elif model_name == MAIN_MODEL_LIST[5]:
            return Viemonet_Polarity_Comp(class_weights=class_weights, label_smoothing=self.label_smoothing)

    def get_foundation_model_name(self, model_name: str):
        assert model_name in MAIN_MODEL_LIST, \
            f"Model {model_name} not recognized. Available models: {MAIN_MODEL_LIST}"
            
        if model_name == MAIN_MODEL_LIST[3]:
            return FOUNDATION_MODEL_LIST[1]  # 'visobert'
        elif model_name == MAIN_MODEL_LIST[4]:
